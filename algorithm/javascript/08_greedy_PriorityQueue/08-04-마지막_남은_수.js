...
function solution(nums) {
    let answer;
    let maxH = new maxHeap(); // 객체 생성.
    for(let x of nums) {
        maxH.insert(x);
    }
    while(maxH.size()>1) { // 1보다 크다라면 계속 줄여나간다.
        let a = maxH.get(); // a가 제일클 꺼고
        let b = maxH.get(); // 두번째로 꺼냈으니 b가 그다음크다.
        if(a!==b) { // 같지않으면 이렇게하고 같다고하면 계속 빼버리면 된다.
            maxH.insert(a-b);
        }
    }
    if(maxH.size() === 0) {// 비어있는 상황
        answer = 0;
    } else { // 뭐가있다.
        answer = maxH.get(); // 하나 남아있는 것.
    }
    return answer;
}
