...
function solution(nums) {
    ...
    function DFS(n, r, wsum, bsum) {
        if(n===r || r===0) {
            max_wsum = Math.max(wsum, max_wsum)
            max_bsum = Math.max(bsum, max_bsum)
            if(max_wsum > max_bsum) console.log(max_wsum-max_bsum)
            else console.log(max_bsum-max_wsum)
        }
        else {
            for(let i=0; i<n; i++) {
                return DFS(n-1, r-1, wsum+nums[i][0], bsum) - DFS(n-1, r, wsum, bsum + nums[i][1])
            }
        }
    }
